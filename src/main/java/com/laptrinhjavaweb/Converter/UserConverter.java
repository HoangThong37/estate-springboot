...
public class UserConverter {
    ...
    public List<StaffAssignmentReponse> convertToStaffAssignmentReponse(List<UserEntity> staffAssignments) {
        List<StaffAssignmentReponse> result = new ArrayList<>();
        for (UserEntity item : userRepository.getAllStaff()) {
            int i = 0;
            ...
            StaffAssignmentReponse staffAssignmentReponse = modelMapper.map(item, StaffAssignmentReponse.class);
            if (i > 0) {
                staffAssignmentReponse.setCheckes("checked");
